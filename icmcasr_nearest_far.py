...
TEXTGRID = ("DA01", "DA02", "DA03", "DA04")


def _parse_utterance(
    corpus_dir: Pathlike,
    section_path: Pathlike,
    text_normalizer: TextNorm,
) -> Optional[Tuple[Recording, SupervisionSegment]]:
    recordings = []
    segments = []
    for textgrid in TEXTGRID:
        text_path = (section_path / (textgrid + ".TextGrid")).resolve()
            ...
            if "name" in dataline:
                ...
                if len(text) > 0:
                    if float(recordings[-1].duration) < end:
                        ...
                        duration = end - start
                    segment_id = recording_id + "-" + textgrid + "-" + str(seq)
                    segments.append(
                        SupervisionSegment(
                            id=segment_id,
...
def _prepare_subset(
    ...
    text_normalizer = TextNorm(to_upper=True)
    ...
    with ThreadPoolExecutor(num_jobs) as ex:
        ...
        for section in tqdm(sections, desc="Distributing tasks"):
            ...
            futures.append(ex.submit(_parse_utterance, corpus_dir, section_path, text_normalizer))
    ...
    for part in tqdm(subsets, desc="Dataset parts"):
        ...
        if output_dir is not None:
            supervision_set.to_file(
                output_dir / f"icmcasr_supervisions_{part}_nearest_far.jsonl.gz"
            )
            recording_set.to_file(output_dir / f"icmcasr_recordings_{part}_nearest_far.jsonl.gz")
...
if __name__ == "__main__":
    prepare_icmcasr("/userhome/data/ICMC-ASR", "/userhome/user/yfy62/icefall-icmcasr/egs/icmcasr/ASR/data", num_jobs=16)
